<%def name="rotate(pagesets, seconds, width, height, left, top, id)">
  ...
  function rotateContent_${id}() {
      rotateUrlIndex_${id} = (rotateUrlIndex_${id} + 1 + 
          Math.floor(Math.random() * (rotateUrls_${id}.length - 1))
          ) % rotateUrls_${id}.length;
      ...
      document.getElementById('rotate_${id}_' + rotateFrameIndex_${id}).contentWindow.postMessage('start', '*');
  ...
  setTimeout(function () {
      rotateContent_${id}();
      setInterval(rotateContent_${id}, ${seconds} * 1000);
  } , 2000);
  //-->
  </script>
</%def>
